---@class Monopuccin
---@field options MonopuccinOptions
---@field setup fun(opts: MonopuccinOptions?)
...
---@class MonopuccinOptions
-- Changes the flavor based on the background. See `:h background` for more info.
---@field background CtpBackground?
-- By default monopuccin writes the compiled results into the system's cache directory.
...
-- Toggle integrations. Integrations allow Monopuccin to set the theme of various plugins.
---@field integrations CtpIntegrations?
-- Monopuccin colors can be overwritten here.
---@field color_overrides CtpColors | CtpFlavors<CtpColors<string>> | nil
-- Monopuccin highlights can be overwritten here.
...
---@class CtpBackground
-- Monopuccin flavor to use when `:set background=dark` is set.
---@field dark CtpFlavor?
-- Monopuccin flavor to use when `:set background=light` is set.
...
--    ui = {
--        kind = require("monopuccin.groups.integrations.lsp_saga").custom_kind(),
--    },
-- }
-- ```
...
---@field enabled boolean
-- Set to a Monopuccin color name to use for the split separator.
