...
local M = {
	default_options = {
		...
		compile_path = vim.fn.stdpath "cache" .. "/monopuccin",
...
function M.compile()
	...
	for flavour, _ in pairs(M.flavours) do
		M.flavour = flavour
		require("monopuccin.lib.compiler").compile(flavour)
	end
...
local function get_flavour(default)
	...
	if flavour and not M.flavours[flavour] then
		vim.notify(
			string.format(
				"Monopuccin (error): Invalid flavour '%s', flavour must be 'latte', 'frappe', 'macchiato', 'mocha' or 'auto'",
				flavour
			),
	...
	return flavour or M.options.flavour or vim.g.monopuccin_flavour or M.options.background[vim.o.background]
end
...
function M.load(flavour)
	...
	f(flavour or M.options.flavour or vim.g.monopuccin_flavour)
end

---@type fun(user_conf: MonopuccinOptions?)
function M.setup(user_conf)
	...
	if user_conf.auto_integrations == true then
		user_conf.integrations = vim.tbl_deep_extend(
			"force",
			require("monopuccin.lib.detect_integrations").create_integrations_table(),
	...
	local hash = require("monopuccin.lib.hashing").hash(user_conf)
...
vim.api.nvim_create_user_command(
	"Monopuccin",
	function(inp) vim.api.nvim_command("colorscheme monopuccin-" .. get_flavour(inp.args)) end,
	{
		nargs = 1,
...
vim.api.nvim_create_user_command("MonopuccinCompile", function()
	for name, _ in pairs(package.loaded) do
		if name:match "^monopuccin." then package.loaded[name] = nil end
	end
	M.compile()
	vim.notify("Monopuccin (info): compiled cache!", vim.log.levels.INFO)
	vim.cmd.colorscheme "monopuccin-nvim"
end, {})

if vim.g.monopuccin_debug then
	vim.api.nvim_create_autocmd("BufWritePost", {
		pattern = "*/monopuccin/*",
		callback = function()
			vim.schedule(function() vim.cmd "MonopuccinCompile" end)
		end,
	})
end
