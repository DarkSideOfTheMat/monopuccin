local path_sep = require("monopuccin").path_sep
local O = require("monopuccin").options
local M = {}
...
function M.compile(flavour)
	local theme = require("monopuccin.lib.mapper").apply(flavour)
	local lines = {
		string.format(
			[[
...
vim.g.colors_name = "monopuccin-%s"
local h = vim.api.nvim_set_hl]],
	...
	if vim.g.monopuccin_debug then -- Debugging purpose
	...
	if not f then
		local err_path = (path_sep == "/" and "/tmp" or os.getenv "TMP") .. "/monopuccin_error.lua"
		print(string.format(
			[[Monopuccin (error): Most likely some mistake made in your monopuccin config
