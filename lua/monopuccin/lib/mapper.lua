...
function M.apply(flavour)
	flavour = flavour or require("monopuccin").flavour

	local _O, _C, _U = O, C, U -- Borrowing global var (setfenv doesn't work with require)
	O = require("monopuccin").options
	C = require("monopuccin.palettes").get_palette(flavour)
	U = require "monopuccin.utils.colors"
	...
	local theme = {}
	theme.editor =
		vim.tbl_deep_extend("force", require("monopuccin.groups.editor").get(), require("monopuccin.groups.lsp").get())

	theme.syntax = {}
	local syntax_modules = { "syntax", "semantic_tokens", "treesitter" }
	for i = 1, #syntax_modules do
		theme.syntax =
			vim.tbl_deep_extend("force", theme.syntax, require("monopuccin.groups." .. syntax_modules[i]).get())
	end
	local final_integrations = {}

	-- https://github.com/monopuccin/nvim/pull/624
	...
	for integration in pairs(O.integrations) do
		...
		if type(O.integrations[integration]) == "table" then
			...
			if O.integrations[integration] == true then
				local default = require("monopuccin").default_options.integrations[integration]
		...
		local ok, result = pcall(require, "monopuccin.groups.integrations." .. integration)
	...
	theme.terminal = require("monopuccin.groups.terminal").get() -- terminal colors
